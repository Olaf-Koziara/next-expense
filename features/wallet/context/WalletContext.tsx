...
import { walletsService } from "../service/wallets";
import { expensesService } from "../../transaction/service/expenses";
import { incomesService } from "../../transaction/service/incomes";
import { Expense } from "@/features/transaction/types/Expense";
import { Income } from "@/features/transaction/types/Income";
...
interface WalletContextType {
  ...
  triggerRefetch: () => void;
  setData: (data: (Expense | Income)[]) => void;
}
...
export function WalletProvider({ children }: { children: React.ReactNode }) {
  ...
  const setData = (data: (Expense | Income)[]) => {
  ...
  useEffect(() => {
    fetchWallets();
    const storedSelectedWalletString =
      window.localStorage.getItem("selectedWallet");
    if (!storedSelectedWalletString) return;
    const storedSelectedWallet = JSON.parse(storedSelectedWalletString);
  ...
  const updateWallet = async (wallet: Wallet) => {
    try {
      ...
      setWallets((prev) =>
        prev.map((w) => (w._id === wallet._id ? wallet : w))
      );
    ...
    if (params) {
      response =
        transactionType === "expense"
          ? await expensesService.getAll(params, selectedWallet?._id)
          : await incomesService.getAll(params, selectedWallet?._id);
    } else {
      response =
        transactionType === "expense"
          ? await expensesService.getAll(selectedWallet?._id)
          : await incomesService.getAll(selectedWallet?._id);
    }
  ...
  const getExpenses = withWallet((walletId: string, params: any = {}) =>
    expensesService.getAll(params, walletId)
  );
  ...
  return (
    <WalletContext.Provider
      value={{
        ...
        isLoading,
        error,
        setSelectedWallet: handleWalletSelect,
